    ...
    <form >
        ...
        <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
            {Object.keys(images).map((key) => (
                <label htmlFor= {`roomImage${key}`} key={key}>
                    <img src= {images[key] ? URL.createObjectURL(images[key]) : assets.uploadArea} alt="room-images" 
        ...
        <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
            ...
            <div className=''>
                ...
                <input value={inputs.pricePerNight} type="number" placeholder='0' onChange={e => setInputs({...inputs, pricePerNight: e.target.value})}
        ...
        <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
            {Object.keys(inputs.amenities).map((amenity, index) => (
                <div key={index}>
                    ...
                    <label className='px-2' htmlFor= {`amenities${index+1}`}>{amenity}</label>
                </div>
            ))}
        </div>
